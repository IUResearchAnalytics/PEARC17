...
system.  As an example, the R environment can be compiled to dynamically link with
...
performance in a high-performance computing environment. Although R Benchmark is the most
...
% JRM - This sentence might need to be reworked after the performance assessments
% are redone.
...
% JRM - Which plots should we include in the paper?  Perhaps 4-6 plots showing
% performance disparities between the two systems for large and small matrix dimensions
%
% JRM - Do we want to include summary numbers and a brief comment on results for
% each and every matrix kernel?
%
%
...
% JRM - Eigendecomposition and the QR have limited scalability because the parallel
% algorithms have data dependencies that limit parallelism.  May one to use
% at most one of these in the plots.
