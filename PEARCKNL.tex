...
% TODO: We need an introductory paragraph, perhaps just two sentences, to
% introduce the overall purpose of what is going to becom a large section.
% TODO: Add a subsection and an appropriate name of the subsection here.
We developed a new R benchmark for HPC environments that tests the run-time
  performance of compute-intensive computational kernels and packages that can
  utilize optimized kernels.
We included computational kernels that are common operations or steps in
  higher-level machine learning and analytics algorithms in R applications.
The computational kernels we included in the HPC benchmark are: Cholesky
  decomposition, eigendecomposition, linear least squares fit, dense linear
...
The QR decomposition, which factors a matrix into an orthogonal matrix and an
  upper triangular matrix, is used in the solution of linear systems and as a
  component of other decompositions such as the eigenvalue
  decomposition~\cite{golub:matrix3, trefethen:numLinAlg}.
...
We have also included benchmark tests of neural network training and cluster
...
The \texttt{nnet} function is used to train neural networks with a single hidden
  ...
  was used to perform the cluster identification.
We included benchmark tests of these packages because neural network training
  and cluster identification can be implemented with computational kernels that
...
Each microbenchmark can be configured to compute run-time performance for
  several problem sizes, where the performance with respect to each problem size
  is computed as an average over a specified number of runs, as the number of
  runs needed to obtain a consistent average may vary with problem size.
...
Our performance testing objectives were to determine the following: which of
  the functions implemented multithreaded parallelism; the single-node
  performance on both the Sandy Bridge and Knights Landing nodes; and an
  estimate of the overhead incurred by R kernel functions when compared to
  similar kernel functionality executed exclusively in C.
Because the kernel functions are used as components in many machine learning and
  analytics algorithms, it was important for us to determine which kernel
...
Because the kernel functions are implemented as wrappers around external C
  function calls, it was worthwhile for us to run performance tests with
  similar kernel functionality exclusively in C to determine how much potential
  there is for performance improvement of the wrapper functions.

\subsection{Description of Tested Systems}
% The tables are omitted for now.  Will probably leave them out to reduce
% page count.
...
Each Sandy Bridge compute node is a Dell PowerEdge C8220z with two Intel Xeon
  E5-2680 (Sandy Bridge architecture) processors with 20 MB of L2 cache each,
  32 GB of DDR3 RAM, and a 61-core Intel Xeon Phi SE10P coprocessor.
The coprocessor has 8 GB of DDR5 RAM and 8 dual-channel controllers, with a peak
  memory bandwidth of 320GB/s.
Table II lists more details about relevant Sandy Bridge compute node
  components~\cite{tacc:stampedeGuide}.  
Each Knights Landing compute node is a Dell PowerEdge with a single 68-core
  Intel Xeon Phi 7250 CPU (Knights Landing architecture), 96 GB of DDR4 RAM, and  16 GB of on-package Multi-Channel Dynamic Random Access Memory (MCDRAM).
The cores in the Xeon Phi CPU are grouped by tiles containing two cores and a
  shared 1 MB L2 cache~\cite{intel:xeonphi}.
The tiles communicate with each other via a two-dimensional mesh network to
...
When the CPU is in flat mode, applications can either explicitly allocate memory
  ...
  numactl to launch the application.
Programs executed with numactl will allocate as much memory from MCDRAM as is
  available; any additional memory needed by the program will be allocated in
  main memory.
When the CPU is in cache mode, the MCDRAM acts as a level 3 cache, operating
  transparently to applications.
Table III lists the relevant Knights Landing compute node
  components~\cite{tacc:stampedeGuide}.
We conducted performance testing with the R programming environment version
  3.2.1 which we downloaded from the Comprehensive R Archive Network (CRAN) on
  both systems.
We built the programming environment using the Intel C compiler and linked with
  the parallel Intel Math Kernel Library (MKL).
