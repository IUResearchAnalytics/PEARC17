...
ggplot(data=scaledata, aes(x=threads, y=scaling, group=interaction(processor,test), color=processor, linetype=test))+
  ...
  coord_cartesian(xlim = c(0, 68)) + labs(list(title="R Matrix Cross Product Scaling", color="Architecture", linetype="Benchmark",
                                               x="Thread Count", y="Runtime in Seconds")) +
  theme(plot.title = element_text(hjust = 0.5)) + scale_color_discrete(labels=c("Knights Landing","SandyBridge"))
...
#  coord_cartesian(xlim = c(0, 68)) + labs(list(title="R Runtimes", color="Architecture", linetype="Benchmark",
...
#                                               x="Thread Count", y="Runtime (sec)")) +
#   theme(plot.title = element_text(hjust = 0.5)) + scale_color_discrete(labels=c("IvyBridge","Knights Landing"))
